...
def save_checkpoint(state, filename='filename.pth.tar'):
...
old= {'state_dict' : model.state_dict(), 'optimizer': optimizer.state_dict()}
old1 = model.state_dict()
# Try load checkpoint
...
for epoch in tqdm(range(EPOCH)): 
    ...
    new = {'state_dict' : model.state_dict(), 'optimizer': optimizer.state_dict()}
    running_loss = 0
...
# In[74]:
model_int = LSTMTagger(embedding_dim=EMBEDDING_DIM, num_layers = num_layers,hidden_dim=HIDDEN_DIM,tagset_size=5,character_size=len(letter_to_ix))


(new['state_dict']['character_embeddings.weight']!=checkpoint['state_dict']['character_embeddings.weight']).sum()
(new['state_dict']['character_embeddings.weight']!=old['state_dict']['character_embeddings.weight']).sum()
(new['state_dict']['character_embeddings.weight']!=old1['character_embeddings.weight']).sum()
(new['state_dict']['character_embeddings.weight']!=model_int.state_dict()['character_embeddings.weight']).sum()
...
# now try two different way to train the model and see if they are identical to debug
#%%
#1. the old way, the seemingly incorrect one
# In[86]:
# Initialize network
model = LSTMTagger(embedding_dim=EMBEDDING_DIM, num_layers = num_layers,hidden_dim=HIDDEN_DIM,tagset_size=5,character_size=len(letter_to_ix))
optimizer = optim.SGD(model.parameters(), learning_rate)
...
for epoch in tqdm(range(EPOCH)): 
    start_time = time.time()
    save_checkpoint(checkpoint,filename='test1.pth.tar')
    running_loss = 0
    for sentence, tags in tqdm(data_train,position=0,leave = True):
        # Step 1. Remember that Pytorch accumulates gradients.
        # We need to clear them out before each instance
...
for epoch in tqdm(range(EPOCH)): 
    start_time = time.time()
    checkpoint = {'state_dict' : model.state_dict(), 'optimizer': optimizer.state_dict()}
    save_checkpoint(checkpoint,filename='test2.pth.tar')
    running_loss = 0
    for sentence, tags in tqdm(data_train,position=0,leave = True):
        # Step 1. Remember that Pytorch accumulates gradients.
        # We need to clear them out before each instance
...
# %%
#load two models
model1 = LSTMTagger(embedding_dim=EMBEDDING_DIM, num_layers = num_layers,hidden_dim=HIDDEN_DIM,tagset_size=5,character_size=len(letter_to_ix))
model2 = LSTMTagger(embedding_dim=EMBEDDING_DIM, num_layers = num_layers,hidden_dim=HIDDEN_DIM,tagset_size=5,character_size=len(letter_to_ix))
optimizer = optim.SGD(model.parameters(), learning_rate)

load_checkpoint(torch.load('test1.pth.tar'), model1, optimizer)
load_checkpoint(torch.load('test2.pth.tar'), model2, optimizer)
#%% check if model1 and model2 has the same parameters.
model1.state_dict()['lstm.weight_hh_l0_reverse']== model2.state_dict()['lstm.weight_hh_l0_reverse']
